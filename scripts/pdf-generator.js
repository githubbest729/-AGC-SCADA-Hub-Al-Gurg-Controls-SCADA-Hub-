...
window.AGC.PDF = (() => {
  ...
  function generateCostEstimationPdf(estimateData, totals) {
    ...
    const htmlString = `
      <div style="padding: 25px; font-family: Arial, sans-serif; color: #1e293b; background: #ffffff; font-size: 9.5pt; width: 740px; box-sizing: border-box;">
        <!-- Header -->
    ...
    // Directly feed the HTML string into html2pdf().from() to avoid any missing element lookup errors
    html2pdf().from(htmlString).set(opt).outputPdf('blob').then((pdfBlob) => {
      ...
      alert("Could not generate PDF quotation. Please try again.");
    });
  }
